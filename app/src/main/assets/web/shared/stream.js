...
BYD.stream = {
    ...
    /**
     * Enable streaming on server. The daemon can reply
     * {success:false, starting:true} while the pipeline/encoder warms;
     * treat that as pending and poll instead of toasting enable_failed.
     */
    async enableStreaming() {
        // Encoder create + GL scaler init while 4 cams are recording can
        // take 15–25s. Keep polling through that window; a one-shot POST
        // is what toasted enable_failed on the HU during a trip.
        const delays = [500, 1000, 2000, 3000, 4000, 5000, 6000, 8000];
        try {
            let data = await this._requestStreamEnable();
            if (data && data.success === true) return true;
            if (!(data && data.starting)) return false;
            if (BYD.utils && BYD.utils.toast) {
                const key = (data.errorCode && typeof data.errorCode === 'string')
                    ? 'stream.' + data.errorCode
                    : 'stream.stream_starting';
                BYD.utils.toast(BYD.i18n.t(key), 'info');
            }
            for (let i = 0; i < delays.length; i++) {
                await new Promise(r => setTimeout(r, delays[i]));
                data = await this._requestStreamEnable();
                if (data && data.success === true) return true;
                if (data && data.success === false && !data.starting) return false;
            }
