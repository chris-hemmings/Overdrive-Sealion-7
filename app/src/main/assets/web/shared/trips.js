...
const TRIPS = {
    ...
    _applyConfigPayload(data) {
        ...
        if (this.rangeFromMs == null && this.trips && this.trips.length > 0
     ...
     * Server returns totals for the selected day window (same cutoff as the
     * trip list), not an ISO-week rollup that can outlive deleted trips.
     */
    ...
    updatePeriodSummary() {
        ...
        // Empty window: zero the tiles rather than leaving stale values from
        // the previously-viewed window or from a leftover weekly rollup.
