...
BYD.core = {
    ...
    async refreshStatus() {
        try {
            ...
            // 12V Battery. On DiLink5, recordingStatus.voltage12v is the live
            // car_service lastEvent (no 30s/60s BatteryMonitor stale gate).
            // Using status.battery alone blanks the tile to "--" whenever that
            // cached block is marked stale, even if car_service has volts.
            {
                const rs12v = status.recordingStatus;
                const isDiLink5 = !!(rs12v && rs12v.dilink5 === true);
                const carSvcVoltage = (rs12v && rs12v.voltage12v !== undefined)
                    ? Number(rs12v.voltage12v) : NaN;
