...
var CHARGING = {
    ...
    tariffGpsLng: null,
    _editingTariff: null,
    // Time-of-use tariffs (day/time-window rates, separate from the
    // location-aware ones above). dayMask bits follow
    // 1 << (Calendar.DAY_OF_WEEK - 1): Sunday=1, Monday=2, ..., Saturday=64.
    timeTariffs: [],
    timeTariffMasterEnabled: false,
    _editingTimeTariff: null,
    _timeTariffDayMask: 0,
    ...
    socHours: 168,            // SoC / power chart window in hours (period selector; default 7d)
    powerHistoryCache: null,
    _powerCacheHours: null,
    ...
    init: function () {
        ...
        document.addEventListener('ot-tabs:active-changed', function (ev) {
            ...
            if (id === 'stats') {
                setTimeout(function () {
                    ...
                    if (self.powerHistoryCache
                            && self._powerCacheHours === self.socHours) {
                        var p = document.getElementById('statsPowerChart');
                        if (p) self.renderStatsPowerCurve(p, self.powerHistoryCache);
                    }
                }, 0);
            }
        });
        ...
        this.loadTimeTariffs();

        var dayPicker = document.getElementById('timeTariffDayPicker');
        if (dayPicker) {
            var dayBtns = dayPicker.querySelectorAll('.ch-day-btn');
            for (var i = 0; i < dayBtns.length; i++) {
                dayBtns[i].addEventListener('click', function (e) {
                    e.preventDefault();
                    this.classList.toggle('active');
                    self._timeTariffDayMask = self._readTimeTariffDayMask();
                });
            }
        }
    },
    ...
    bootstrap: function () {
        ...
        var powerGeneration = ++this._powerGeneration;
        var sessionsGeneration = ++this._sessionsGeneration;
        ...
        return this._fetchJson(this._bootstrapUrl(periodKey, hours)).then(function (data) {
            ...
            var power = self._payload(b.power, 'power', true, false);
            ...
            if (powerGeneration === self._powerGeneration
                    && hours === self.socHours) {
                if (power !== null) self._applyPower(power, hours);
                else followups.push(self.loadPower());
            }
    ...
    loadPower: function () {
        var self = this;
        var hours = this.socHours || 168;
        var generation = ++this._powerGeneration;
        return this._fetchJson('/api/charging/power?hours=' + hours + '&points=300')
            .then(function (d) {
                if (generation !== self._powerGeneration
                        || hours !== self.socHours) return;
                var power = self._payload(d, 'power', true, true);
                if (power === null) throw new Error('invalid power payload');
                self._applyPower(power, hours);
            })
            .catch(function () {});
    },
    ...
    _applyPower: function (power, hours) {
        if (!power) return;
        var effectiveHours = hours != null
            ? hours : this._powerCacheHours;
        if (effectiveHours == null) effectiveHours = this.socHours;
        if (effectiveHours !== this.socHours) return;
        this.powerHistoryCache = power;
        this._powerCacheHours = effectiveHours;
        var c = document.getElementById('statsPowerChart');
        if (c) this.renderStatsPowerCurve(c, power);
    },
    ...
    // Scale the kW axis to the data. A 1.4 kW AC session used to be ceil'd
    // onto a 10 kW axis and looked like a flat line on the floor.
    _nicePowerMax: function (maxP) {
        if (!(maxP > 0) || !isFinite(maxP)) return 2;
        if (maxP <= 2) return 2;
        if (maxP <= 5) return Math.ceil(maxP * 2) / 2;
        if (maxP <= 22) return Math.ceil(maxP);
        return Math.ceil(maxP / 10) * 10;
    },

    _powerTickLabel: function (v, maxP) {
        if (maxP <= 5) return (Math.round(v * 10) / 10).toFixed(1);
        return String(Math.round(v));
    },

    _statsPowerPoints: function (history) {
        var GAP_MS = 45 * 60 * 1000;
        var pts = [];
        ...
        for (var i = 0; i < history.length; i++) {
            ...
            if (pts.length && ((s.session != null && s.session !== lastSession)
                    || s.t - lastT > GAP_MS)) {
                pts.push({ t: lastT + 1, power: null });
            }
            pts.push({ t: s.t, power: p });
    ...
    // Stats-tab kW-over-time. Independent segments per session so idle gaps
    // are not drawn as a diagonal. Hover lists kW + clock time.
    renderStatsPowerCurve: function (canvas, history) {
        if (!canvas) return;
        var samples = this._statsPowerPoints(history);
        var measured = 0;
        for (var n = 0; n < samples.length; n++) {
            if (samples[n] && samples[n].power != null) measured++;
        }
        var note = document.getElementById('statsPowerNoSamples');
        if (measured < 2) {
        ...
        var plotW = w - pad.l - pad.r, plotH = h - pad.t - pad.b;
        var t0 = samples[0].t;
        var tSpan = (samples[samples.length - 1].t - t0) || 1;
        var maxP = 0;
        for (var i = 0; i < samples.length; i++) {
            if (samples[i] && samples[i].power > maxP) maxP = samples[i].power;
        }
        maxP = this._nicePowerMax(maxP);
        var x = function (t) { return pad.l + ((t - t0) / tSpan) * plotW; };
        var yP = function (v) { return pad.t + (1 - (v / maxP)) * plotH; };
        var self = this;
        this._drawGrid(ctx, pad, plotW, plotH,
            [0, maxP * 0.25, maxP * 0.5, maxP * 0.75, maxP],
            function (v) { return self._powerTickLabel(v, maxP); }, maxP, 0);
        ...
        var tMax = samples[samples.length - 1].t;
        var longSpan = (tMax - t0) > 20 * 60 * 60 * 1000;
        ctx.fillStyle = this.colors.textMuted;
        ctx.font = '10px Inter, sans-serif';
        ctx.textBaseline = 'top';
        ctx.textAlign = 'left';
        ctx.fillText(longSpan ? this._fmtDate(t0) : this._fmtClock(t0), pad.l, pad.t + plotH + 6);
        ctx.textAlign = 'right';
        ctx.fillText(longSpan ? this._fmtDate(tMax) : this._fmtClock(tMax), pad.l + plotW, pad.t + plotH + 6);

        var selfP = this;
        this._registerDetailHover(canvas, {
            samples: samples, t0: t0, tSpan: tSpan, pad: pad, plotW: plotW, plotH: plotH, w: w, h: h,
            render: function () {
                selfP.renderStatsPowerCurve(canvas, selfP.powerHistoryCache);
            },
            dot: function (c, p, px) {
    ...
    renderPowerCurve: function (canvas, samples) {
        ...
        // Power area + line. Build segments from samples that actually carry a
        // power reading, breaking only on a genuinely large time gap between
        // two real readings (GENUINE_GAP_MS) rather than on every null row.
        // Power and SoC are sampled independently, so most rows alternate
        // real-power / null-power on a routine ~5-30s cadence — breaking on
        // every null shattered nearly every reading into an isolated 1-point
        // "segment", and a moveTo with no following lineTo draws nothing: the
        // power line was present in the data but invisible on screen (the
        // SoC dashed line below has no such break and rendered fine). A real
        // multi-minute gap (charger unplugged, counter dead) still breaks the
        // line so no rate is invented across it.
        var grad = ctx.createLinearGradient(0, pad.t, 0, pad.t + plotH);
        grad.addColorStop(0, this._rgba(this.colors.brand, 0.30));
        grad.addColorStop(1, this._rgba(this.colors.brand, 0.02));
        var GENUINE_GAP_MS = 5 * 60 * 1000;
        var realPoints = [];
        for (var rp0 = 0; rp0 < samples.length; rp0++) {
            if (samples[rp0] != null && samples[rp0].power != null) {
                realPoints.push(samples[rp0]);
            }
        }
        var segments = [], segment = [];
        for (var j = 0; j < realPoints.length; j++) {
            if (segment.length
                    && (realPoints[j].t - segment[segment.length - 1].t) > GENUINE_GAP_MS) {
                segments.push(segment);
                segment = [];
            }
            segment.push(realPoints[j]);
        }
        if (segment.length) segments.push(segment);
        for (var sg = 0; sg < segments.length; sg++) {
            var run = segments[sg];
    ...
    // ==================== TIME-OF-USE TARIFFS (day/time-window rates) ====================

    loadTimeTariffs: function () {
        var self = this;
        return this._fetchJson('/api/vehicle/diag/time-tariffs')
            .then(function (d) {
                if (!d || d.success !== true || !Array.isArray(d.tariffs)) {
                    throw new Error('invalid time-tariffs payload');
                }
                self.timeTariffMasterEnabled = !!d.enabled;
                self.timeTariffs = d.tariffs;
                self.renderTimeTariffs();
                self._setVal('timeTariffMasterToggle', self.timeTariffMasterEnabled, true);
            })
            .catch(function () {});
    },

    renderTimeTariffs: function () {
        var list = document.getElementById('timeTariffList');
        var empty = document.getElementById('timeTariffEmpty');
        if (!list) return;

        var rows = this.timeTariffs || [];
        if (this._editingTimeTariff) {
            var stillThere = false;
            for (var k = 0; k < rows.length; k++) {
                if (rows[k].id === this._editingTimeTariff.id) { stillThere = true; break; }
            }
            if (!stillThere) this.closeTimeTariffEditor();
        }
        list.innerHTML = '';
        if (rows.length === 0) {
            if (empty) empty.style.display = '';
        ...
        var self = this;
        for (var i = 0; i < rows.length; i++) {
            list.appendChild(self._timeTariffRow(rows[i]));
        }
    },

    _dayShortLabels: ['common.day_sun_short', 'common.day_mon_short', 'common.day_tue_short',
        'common.day_wed_short', 'common.day_thu_short', 'common.day_fri_short', 'common.day_sat_short'],
    _dayShortFallback: ['S', 'M', 'T', 'W', 'T', 'F', 'S'],

    // dayMask bit i (0-based) corresponds to Calendar.DAY_OF_WEEK i+1: bit0=Sun ... bit6=Sat.
    _fmtDayMask: function (mask) {
        var out = [];
        for (var i = 0; i < 7; i++) {
            if (mask & (1 << i)) {
                out.push(this._esc(this._t(this._dayShortLabels[i], this._dayShortFallback[i])));
            }
        }
        return out.length === 7 ? this._t('charge.time_tariff_every_day', 'Every day') : out.join(' ');
    },

    _fmtMinuteOfDay: function (m) {
        m = ((m % 1440) + 1440) % 1440;
    ...
    _timeTariffRow: function (t) {
        var self = this;
        var row = document.createElement('div');
        row.className = 'ch-tariff-row' + (t.enabled === false ? ' disabled' : '');

        var rateBits = [];
        if (t.acRate > 0) {
            rateBits.push('<span class="mono">' + this._esc(this.currency || '$') + t.acRate.toFixed(2) + '</span> '
                + this._esc(this._t('charge.tariff_ac_short', 'AC')));
        }
        if (t.dcRate > 0) {
            rateBits.push('<span class="mono">' + this._esc(this.currency || '$') + t.dcRate.toFixed(2) + '</span> '
                + this._esc(this._t('charge.tariff_dc_short', 'DC')));
        }
        if (rateBits.length === 0) {
            rateBits.push(this._esc(this._t('charge.tariff_no_rate', 'No rate set')));
        ...
        var label = (t.label && t.label !== '')
            ? t.label : this._t('charge.tariff_unnamed', 'Unnamed tariff');
        var windowText = this._fmtMinuteOfDay(t.startMinute) + '–' + this._fmtMinuteOfDay(t.endMinute);

        row.innerHTML =
            '<div class="tariff-info">' +
                '<div class="tariff-name-row"><span class="tariff-name">' + this._esc(label) + '</span></div>' +
                '<div class="tariff-meta">' + rateBits.join(' · ') + '</div>' +
                '<div class="tariff-sub">' + this._esc(this._fmtDayMask(t.dayMask)) + ' · ' + this._esc(windowText) + '</div>' +
            '</div>' +
            '<div class="ch-tariff-actions">' +
                '<button data-act="edit">' + this._esc(this._t('common.edit', 'Edit')) + '</button>' +
                '<button data-act="delete">' + this._esc(this._t('common.delete', 'Delete')) + '</button>' +
            '</div>';

        var btns = row.querySelectorAll('.ch-tariff-actions button');
        for (var i = 0; i < btns.length; i++) {
            (function (btn) {
                btn.addEventListener('click', function (e) {
                    e.stopPropagation();
                    var act = btn.getAttribute('data-act');
                    if (act === 'edit') self.openTimeTariffEditor(t);
                    else if (act === 'delete') self.deleteTimeTariff(t);
                });
            })(btns[i]);
        }
        return row;
    ...
    _readTimeTariffDayMask: function () {
        var mask = 0;
        var picker = document.getElementById('timeTariffDayPicker');
        if (!picker) return mask;
        var btns = picker.querySelectorAll('.ch-day-btn.active');
        for (var i = 0; i < btns.length; i++) {
            mask |= (parseInt(btns[i].getAttribute('data-daybit'), 10) || 0);
        }
        return mask;
    },

    _paintTimeTariffDayMask: function (mask) {
        var picker = document.getElementById('timeTariffDayPicker');
        if (!picker) return;
        var btns = picker.querySelectorAll('.ch-day-btn');
        for (var i = 0; i < btns.length; i++) {
            var bit = parseInt(btns[i].getAttribute('data-daybit'), 10) || 0;
            if (mask & bit) btns[i].classList.add('active');
            else btns[i].classList.remove('active');
        }
    },

    /**
     * Open the add/edit panel. `t` null = create (defaults to every day, 22:00-06:00);
     * an object = edit that tariff.
     */
    openTimeTariffEditor: function (t) {
        var editor = document.getElementById('timeTariffEditor');
        if (!editor) return;
        this._editingTimeTariff = t || null;

        this._setInput('timeTariffLabelInput', t ? (t.label || '') : '');
        this._setInput('timeTariffAcRateInput', t && t.acRate > 0 ? t.acRate : '');
        this._setInput('timeTariffDcRateInput', t && t.dcRate > 0 ? t.dcRate : '');
        this._setInput('timeTariffPriorityInput', t ? (t.priority || 0) : 0);
        this._setInput('timeTariffStartInput', this._fmtMinuteOfDay(t ? t.startMinute : 22 * 60));
        this._setInput('timeTariffEndInput', this._fmtMinuteOfDay(t ? t.endMinute : 6 * 60));
        var mask = t ? t.dayMask : 127;
        this._timeTariffDayMask = mask;
        this._paintTimeTariffDayMask(mask);

        this._setTimeTariffError('');
        editor.style.display = '';
        var labelInput = document.getElementById('timeTariffLabelInput');
        if (labelInput) { try { labelInput.focus(); } catch (e) {} }
    },

    closeTimeTariffEditor: function () {
        var editor = document.getElementById('timeTariffEditor');
        if (editor) editor.style.display = 'none';
        this._editingTimeTariff = null;
        this._setTimeTariffError('');
    },

    _setTimeTariffError: function (msg) {
        var el = document.getElementById('timeTariffError');
        if (!el) return;
        if (!msg) { el.style.display = 'none'; el.textContent = ''; return; }
        el.textContent = msg;
        el.style.display = '';
    ...
    saveTimeTariff: function () {
        var self = this;
        var editing = this._editingTimeTariff;
        var label = this._getStr('timeTariffLabelInput').trim();
        var acRate = this._getNum('timeTariffAcRateInput');
        var dcRate = this._getNum('timeTariffDcRateInput');
        var priority = this._getNum('timeTariffPriorityInput') || 0;
        var dayMask = this._readTimeTariffDayMask();
        var startMinute = this._parseMinuteInput('timeTariffStartInput');
        var endMinute = this._parseMinuteInput('timeTariffEndInput');

        if (acRate <= 0 && dcRate <= 0) {
            this._setTimeTariffError(this._t('charge.tariff_err_no_rate', 'Enter an AC or DC rate'));
            return;
        }
        if (acRate < 0 || dcRate < 0 || acRate >= 100000 || dcRate >= 100000) {
            this._setTimeTariffError(this._t('charge.tariff_err_rate_range', 'Rates must be between 0 and 100000'));
            return;
        }
        if (dayMask === 0) {
            this._setTimeTariffError(this._t('charge.time_tariff_err_no_days', 'Select at least one day'));
            return;
        }
        if (startMinute < 0 || endMinute < 0) {
            this._setTimeTariffError(this._t('charge.time_tariff_err_window', 'Enter a valid start and end time'));
            return;
        }
        if (label.length > 48) {
            this._setTimeTariffError(this._t('charge.tariff_err_label', 'Label is too long'));
            return;
        }
        ...
        var btn = document.getElementById('timeTariffSaveBtn');
        if (btn) { btn.disabled = true; btn.textContent = this._t('common.saving', 'Saving…'); }

        fetch('/api/vehicle/diag/time-tariffs/save', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify(body)
        }).then(function (r) { return r.json(); })
          .then(function (d) {
              if (btn) { btn.disabled = false; btn.textContent = self._t('common.save', 'Save'); }
              if (!d || !d.success) {
                  self._setTimeTariffError((d && d.error) || self._t('charge.tariff_err_save', 'Could not save tariff'));
                  return;
              }
              self.closeTimeTariffEditor();
              self._toast(self._t('charge.tariff_saved', 'Tariff saved'));
              self.loadTimeTariffs();
          })
          .catch(function () {
              if (btn) { btn.disabled = false; btn.textContent = self._t('common.save', 'Save'); }
              self._setTimeTariffError(self._t('charge.tariff_err_save', 'Could not save tariff'));
          });
    },

    deleteTimeTariff: function (t) {
        var self = this;
        var ask = this._t('charge.tariff_confirm_delete', 'Delete this tariff? Charges priced with it will fall back to your other rates.');
        var proceed = function (ok) {
            if (!ok) return;
            fetch('/api/vehicle/diag/time-tariffs/delete', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ id: t.id })
            }).then(function (r) { return r.json(); })
              .then(function (d) {
                  if (d && d.success) {
                      self._toast(self._t('charge.tariff_deleted', 'Tariff deleted'));
                      self.loadTimeTariffs();
                  } else {
                      self._toast(self._t('charge.tariff_err_delete', 'Could not delete tariff'), 'error');
                  }
    ...
    setTimeTariffMasterToggle: function (enabled) {
        var self = this;
        var prev = this.timeTariffMasterEnabled;
        fetch('/api/vehicle/diag/time-tariffs/toggle', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
          ...
          .then(function (d) {
              if (d && d.success) {
                  ...
                  self.timeTariffMasterEnabled = prev;
                  self._setVal('timeTariffMasterToggle', prev, true);
                  self._toast(self._t('charge.tariff_err_save', 'Could not save tariff'), 'error');
              }
          })
          .catch(function () {
