...
(function () {
    ...
    // Patch fetch() HERE (head, before any page script) so Live View's
    // auto-start POST /api/stream/enable hits AndroidBridge. Waiting for
    // WebViewFragment.onPageFinished is too late: i18n.init() then
    // selectCamera(0) races the inject, the intercept used to rewrite that
    // POST as GET, and the in-app page toasted enable_failed while a
    // Tailscale browser (real POST) worked. Idempotent with INJECT_JS.
    if (IN_APP_WEBVIEW && window.fetch && !window._fetchPatched) {
        window._fetchPatched = true;
        var _origFetch = window.fetch;
        window.fetch = function (input, init) {
            init = init || {};
            var url = (typeof input === 'string') ? input : (input.url || '');
            var isLocal = url.charAt(0) === '/' || url.indexOf('127.0.0.1') !== -1
                || url.indexOf('localhost') !== -1;
            var isApi = url.indexOf('/api/') !== -1 || url.indexOf('/auth/') !== -1;
            if (!isLocal || !isApi) return _origFetch.call(window, input, init);
            var method = (init.method || 'GET').toUpperCase();
            if (method === 'GET') return _origFetch.call(window, input, init);
            var body = init.body || '';
            var headers = {};
            if (init.headers) {
            ...
            var fullUrl = url.charAt(0) === '/' ? 'http://127.0.0.1:8080' + url : url;
