...
public class GpuSurveillancePipeline {
    ...
    // GpuStreamScaler.init() is posted onto the camera GL thread. Idle that
    // queue drains in well under 2s. During 4-cam recording the same thread
    // is already mosaic+encode-busy, so a 2s wait throws "Stream scaler
    // initialization timed out" and Live View toasts enable_failed — while a
    // Tailscale browser that retries / waits longer still comes up. Give the
    // GL queue a full record-loop budget when a recorder is live.
    private static final long STREAM_SCALER_INIT_WAIT_MS = 2_000L;
    private static final long STREAM_SCALER_INIT_WAIT_RECORDING_MS = 12_000L;
    ...
    private void enableStreamingInternal(int streamWidth, int streamHeight, int streamFps,
        ...
        // a 12-second recording wait with the GL thread mid-shader-compile
        ...
        // Wait for GL thread initialization — HOLDING streamLifecycleLock
        // (audit follow-up 2). An earlier fix released the lock here so
        // peers didn't pin for the full wait, reasoning that a concurrent
        // disableStreaming "sees streamingEnabled == false and bails —
        // safe". That reasoning missed the camera CLOSE path: the stream
        // encoder and its drainer are ALREADY LIVE (streamEncoder.init()
        ...
        // disableStreaming() unconditionally, and blocking it here is exactly
        // the serialization that makes its camera close safe.
        long glWaitMs = STREAM_SCALER_INIT_WAIT_MS;
        try {
            if (isRecording() || recordingMode) {
                glWaitMs = STREAM_SCALER_INIT_WAIT_RECORDING_MS;
            }
        } catch (Throwable ignored) {}
        synchronized (initLock) {
            if (!initDone[0]) {
                initLock.wait(glWaitMs);
            }
        }

        if (!initDone[0]) {
            throw new RuntimeException("Stream scaler initialization timed out after "
                + glWaitMs + "ms");
        }
