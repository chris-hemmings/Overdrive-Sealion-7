...
public class SurveillanceEngineGpu {
    ...
    private void processFrameV2(byte[] smallRgbFrame, long now) {
        ...
        try {
            int walkQ = pipelineV2.getHighestThreatQuadrant();
            String cam = (walkQ >= 0 && walkQ < MotionPipelineV2.QUADRANT_NAMES.length)
                    ? MotionPipelineV2.QUADRANT_NAMES[walkQ] : "-";
            String threat = "NONE";
            if (maxThreat >= MotionPipelineV2.THREAT_HIGH) threat = "HIGH(loiter)";
            else if (maxThreat >= MotionPipelineV2.THREAT_MEDIUM) threat = "MEDIUM(approach)";
            else if (maxThreat >= MotionPipelineV2.THREAT_LOW) threat = "LOW(pass)";
            String prox = "-";
            if (walkQ >= 0 && results[walkQ] != null) {
                prox = proximityForQuadrant(walkQ, results[walkQ]).describe();
            }
            ScreenDeterrent sd = ScreenDeterrent.getInstance();
            boolean wake = SentryScreenWalkLog.consumeNewApproach(maxThreat);
            if (wake) sd.onMotionDetected();
            SentryScreenWalkLog.tick(
                    wake || sd.isSessionActive(),
                    sd.isSessionActive(),
                    sd.isEnabledCached(),
                    cam, threat, prox, recording, anyMotion);
        } catch (Throwable ignored) {}
        ...
        if (anyMotion) {
            ...
            if (motionDuration >= requiredDuration) {
                ...
                if (!recording) {
                    ...
                    // Cloud can re-fire on its own cooldown. Screen is one 8s
                    // burst per approach (walk-tick consumeNewApproach) so an
                    // in-progress clip does not keep the panel up.
                    try {
                        com.overdrive.app.byd.cloud.BydCloudDeterrent.getInstance().onMotionDetected();
    ...
    /** True while a CPU-XNNPACK YOLO {@code detect()} is on aiExecutor. */
    public boolean isInferenceInFlight() {
        return isAiRunning.get();
    }

    /**
     * One-line sentry/AI/motion snapshot for GL-thread stall logs. Motion age
     * uses the last wall-clock stamp that any-motion was observed; -1 means
     * no motion this arm session.
     */
    public String stallDiagnostics() {
        long now = android.os.SystemClock.elapsedRealtime();
        long motionAgeMs = (lastMotionActiveWallMs == 0L)
                ? -1L
                : Math.max(0L, now - lastMotionActiveWallMs);
        return "sentry=" + active
                + " cont=" + continuousMode
                + " infer=" + isAiRunning.get()
