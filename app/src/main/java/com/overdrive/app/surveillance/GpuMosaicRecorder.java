...
public class GpuMosaicRecorder {
    ...
    // Immediate stall log (complements the 30 s DrawStage worst-frame roll-up).
    // 150 ms is the freeze-and-skip threshold measured in cam_* stts; ≥500 ms
    // always logs, 150–500 ms is 1 Hz so chronic stutter stays readable.
    private static final long STALL_LOG_DRAW_NS = 150_000_000L;
    private static final long STALL_LOG_ALWAYS_NS = 500_000_000L;
    private static final long STALL_LOG_MIN_INTERVAL_MS = 1_000L;
    private long lastDrawStallLogMs = 0L;
    private int suppressedDrawStallLogs = 0;
    ...
    public void drawFrame(int cameraTextureId, int windshieldTextureId,
        ...
        if (elapsedNs >= STALL_LOG_DRAW_NS) {
            maybeLogDrawStall(elapsedNs, stageRtNs, stageSwapNs, stageOverlayNs, stageShaderNs);
        }
    ...
    private void maybeLogDrawStall(long elapsedNs, long rtNs, long swapNs,
                                   long overlayNs, long shaderNs) {
        long now = System.currentTimeMillis();
        boolean always = elapsedNs >= STALL_LOG_ALWAYS_NS;
        if (!always && (now - lastDrawStallLogMs) < STALL_LOG_MIN_INTERVAL_MS) {
            suppressedDrawStallLogs++;
            return;
        }
        int suppressed = suppressedDrawStallLogs;
        suppressedDrawStallLogs = 0;
        lastDrawStallLogMs = now;
        HardwareEventRecorderGpu enc = encoder;
        String snap = (enc != null) ? enc.stallSnapshot() : "encoder=null";
        logger.warn("Draw stall: total=" + (elapsedNs / 1_000_000L)
                + "ms rt=" + (rtNs / 1_000_000L)
                + "ms swap=" + (swapNs / 1_000_000L)
                + "ms ovl=" + (overlayNs / 1_000_000L)
                + "ms shd=" + (shaderNs / 1_000_000L)
                + "ms " + snap
