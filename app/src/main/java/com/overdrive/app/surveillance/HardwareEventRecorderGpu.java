...
public class HardwareEventRecorderGpu {
    ...
    // ── A/B TEST TOGGLE ──────────────────────────────────────────────────
    // KEY_OPERATING_RATE pin master switch. TRUE: the encoder holds Venus /
    // GPU at a committed fps so the SoC governor cannot DVFS-down between
    // frames. That was added in v18.1 after 100-200 ms eglSwap stalls landed
    // in the MP4 as freeze-and-skip. It was later flipped FALSE because a
    // pinned encode made the un-pinned BYD UI lose the clock lottery (smooth
    // recording, laggy head unit).
    //
    // Flipped back to TRUE after Sealion 7 / DiLink5 cam_* clips still showed
    // stts gaps from ~40 ms to 2.5 s (~30% of frames preceded by >200 ms) with
    // FUSE staging and a fast SD card already in place. Storage and the
    // cam-daemon crash-loop were ruled out; the remaining mechanism this pin
    // addresses is Venus dropping clocks mid-GOP. Cost: cabin UI may hitch
    // while we record. Secondary encoders (OEM dashcam, live stream) still
    // force this off via setPinOperatingRate(false) so two sessions cannot
    // over-subscribe the video block.
    //   true  = (current) pin at fps — smoother OUR video, hotter SoC.
    //   false = let Venus DVFS down between frames — cooler SoC, risk of
    //           100-200 ms+ eglSwap stalls in the recording.
    private static final boolean PIN_OPERATING_RATE = true;
    ...
    private boolean writeRebased(android.media.MediaMuxer mux, int trackIdx,
        ...
        if (lastSourcePtsUs >= 0) {
            ...
            } else if (sourceGap >= STALL_LOG_GAP_US) {
                maybeLogInterframeStall(sourceGap);
            }
        }
    ...
    // ==================== FAST STAGING (FUSE-bypass) ====================
    //
    // Live recording writes MUST NOT target /storage/emulated/0/... directly.
    // Measured on-device: /storage/emulated (scoped-storage FUSE) sustains
    // ~6 MB/s sequential write, while /data/local/tmp on the SAME underlying
    // UFS flash sustains ~775 MB/s — a ~127x penalty from the FUSE layer
    // alone, far beyond FUSE's normal 10-30% overhead. An 8 Mbps event
    // recording needs ~1 MB/s sustained; against a 6 MB/s ceiling that leaves
    // almost no margin, and any transient burst (segment rotation, a GC
    // pause, another daemon writing concurrently) pushes the required rate
    // over what FUSE can sustain. The muxer write-queue then backs up, which
    // backpressures the GL/camera pipeline (see the eglSwapBuffers stall
    // notes throughout this file) — producing multi-hundred-ms to
    // multi-second freezes baked directly into the recording (field-measured
    // up to 2.5s gaps between consecutive frames in a single clip).
    //
    // The fix: the muxer writes to a staging file on the fast, non-FUSE
    // /data/local/tmp path (same convention already used for the charging
    // and trip H2 databases) for the ENTIRE live recording. Only once a
    // segment is fully stopped and closed — no longer time-sensitive — does
    // finalizeToDestination() copy it to its real /storage/emulated/...
    // destination and delete the staging copy. A copy of an already-finished
    // file taking a moment doesn't cause playback stutter; a live encoder
    // blocked on a slow write does.
    private static final String STAGING_DIR = "/data/local/tmp/overdrive_recording_staging";

    /** Fast-storage staging path for a given final output path's temp file. */
    private File stagingFileFor(String finalOutputPath) {
        ...
        return new File(dir, new File(finalOutputPath).getName() + ".tmp");
    }

    /**
     * Copies a fully-written staging file to its final (possibly
     * FUSE-mediated) destination, then deletes the staging copy. Unlike
     * {@code File.renameTo()} — which only works within a single filesystem
     * and silently fails across mount points — this does a real byte copy,
     * landing it atomically at {@code finalDestination} via a same-volume
     * temp name + rename so nothing ever observes a partially-written final
     * file. Safe to call from any thread; callers on the encoder/GL hot path
     * must not call this synchronously (see the background-thread finalizer
     * for the segment-rotation case) — a finished segment's copy is not
     * time-sensitive, but it should still never block live encoding.
     *
     * @return true if the final file now exists with the staging file's
     *         complete contents and the staging copy was removed.
     */
    private boolean finalizeToDestination(File stagingFile, File finalDestination) {
        if (stagingFile == null || !stagingFile.exists()) return false;
        File parentDir = finalDestination.getParentFile();
        if (parentDir != null && !parentDir.exists()) {
            parentDir.mkdirs();
        }
        File xfer = new File(parentDir, finalDestination.getName() + ".xfer");
        long expectedLength = stagingFile.length();
        try (java.io.FileInputStream fis = new java.io.FileInputStream(stagingFile);
             java.io.FileOutputStream fos = new java.io.FileOutputStream(xfer)) {
            ...
            while (transferred < expectedLength) {
                long chunk = inChannel.transferTo(transferred, expectedLength - transferred, outChannel);
                if (chunk <= 0L) break;
                transferred += chunk;
            }
            if (transferred != expectedLength) {
                logger.error("Staging copy incomplete for " + finalDestination.getName()
                        + " (" + transferred + "/" + expectedLength + " bytes)");
                xfer.delete();
                return false;
            }
        } catch (Exception e) {
            logger.error("Staging->final copy failed for " + finalDestination.getName()
                    + ": " + e.getMessage());
            xfer.delete();
            return false;
        }
        if (xfer.length() != expectedLength || !xfer.renameTo(finalDestination)) {
            logger.error("Final rename failed for " + finalDestination.getName());
            xfer.delete();
            return false;
        }
        if (!stagingFile.delete()) {
            logger.warn("Failed to delete staging file after copy: " + stagingFile.getName());
        }
        return true;
    }
    ...
    // Playback-stutter probe. MAX_PLAUSIBLE_INTERFRAME_GAP_US only re-anchors
    // clock-domain jumps (≥10 s). The freeze-and-skip the user actually sees
    // is 150 ms–2.5 s baked into stts — log those here with encoder/muxer/GC
    // context so the next clip's daemon log correlates with the mp4 timestamps.
    private static final long STALL_LOG_GAP_US = 150_000L;
    private static final long STALL_LOG_ALWAYS_US = 500_000L;
    private static final long STALL_LOG_MIN_INTERVAL_MS = 1_000L;
    private long lastInterframeStallLogMs = 0L;
    private int suppressedInterframeStallLogs = 0;
    ...
    public void init() throws Exception {
        ...
        if (pinOperatingRate) {
            ...
            logger.info("Encoder OPERATING_RATE pin skipped (disabled for this encoder)");
        }
    ...
    public boolean triggerEventRecording(String outputPath, long postRecordDurationMs,
        ...
        try {
            ...
            // Write to a fast staging file during recording — NOT the final
            // /storage/emulated/... destination, which is FUSE-mediated and
            // measured at ~6 MB/s on this hardware (see STAGING_DIR javadoc).
            tempFile = stagingFileFor(outputPath);
    ...
    private void closeEventRecording() {
        ...
        // Snapshot everything the finalizer needs into locals BEFORE
        // dispatching to a background thread and before the "Reset state"
        // block below reassigns these instance fields for the next
        // recording — mirrors the rotation finalizer's old*/closedGeoAtCommit
        // snapshot pattern. Without this the async finalizer could read the
        // NEXT recording's pts/segment/geo values instead of the ones for the
        // recording it's actually finalizing.
        //
        // Backgrounding this is itself a fix: finalizeToDestination() is a
        // full-file copy (it stages through /data/local/tmp to dodge the FUSE
        // write penalty), not a cheap rename. Running it synchronously here
        // — still holding startStopLock, per stopEventRecording's docstring —
        // meant a slow copy to a FUSE-mounted or actual SD/USB destination
        // blocked that lock for the copy's full duration. triggerEventRecording
        // takes the same lock, so a new sentry/proximity trigger arriving
        // mid-copy would stall waiting for it — reintroducing the kind of
        // coverage gap the staging fix exists to prevent. The rotation-path
        // finalizer already backgrounds its copy the same way; this matches it.
        final File tempFileSnapshot = tempFile;
        final String outputPathSnapshot = outputPath;
        final int recordedFramesSnapshot = recordedFrames;
        final long firstFramePtsUsSnapshot = firstFramePtsUs;
        final long lastFramePtsUsSnapshot = lastFramePtsUs;
        final int fpsSnapshot = fps;
        final int segmentNumberSnapshot = segmentNumber;
        final String codecMimeTypeSnapshot = codecMimeType;
        final int bitrateSnapshot = bitrate;
        final boolean recordingBrokenSnapshot = recordingBroken;
        final boolean stopOkSnapshot = stopOk;
        final boolean abortedAtCloseSnapshot = abortedAtClose;
        final boolean hasStartGeoSnapshot = hasStartGeo();
        final double startGeoLatSnapshot = startGeoLat;
        final double startGeoLngSnapshot = startGeoLng;
        final float startGeoAccuracySnapshot = startGeoAccuracy;
        final long startGeoAgeMsSnapshot = startGeoAgeMs;
        final long startGeoCapturedAtMsSnapshot = startGeoCapturedAtMs;
        ...
        if (tempFileSnapshot != null) {
            ...
            Thread finalizerThread = new Thread(() -> {
              try {
                ...
                if (!tempFileSnapshot.exists()) {
                    return;
                }
                if (!recordingBrokenSnapshot && recordedFramesSnapshot > 0 && tempFileSnapshot.length() > 1024) {
                    File finalFile = new File(outputPathSnapshot);
                    if (finalizeToDestination(tempFileSnapshot, finalFile)) {
                        // Use actual PTS range for accurate duration (not recordedFrames/fps
                        // which is misleading when pre-record frames are included)
                        float durationSec = (firstFramePtsUsSnapshot >= 0 && lastFramePtsUsSnapshot > firstFramePtsUsSnapshot)
                                ? (lastFramePtsUsSnapshot - firstFramePtsUsSnapshot) / 1_000_000.0f
                                : recordedFramesSnapshot / (float) fpsSnapshot;
                        ...
                        logger.info(String.format("Event saved: %s (segment %d, %d frames, %.1f sec, %d KB, codec=%s, bitrate=%d Mbps)",
                                finalFile.getName(), segmentNumberSnapshot, recordedFramesSnapshot, durationSec, finalFile.length() / 1024,
                                codecMimeTypeSnapshot.equals(MediaFormat.MIMETYPE_VIDEO_HEVC) ? "H.265" : "H.264",
                                bitrateSnapshot / 1_000_000));
                        ...
                        try {
                            ...
                            if (!"surveillance".equals(flow)) {
                                ...
                                if (hasStartGeoSnapshot) {
                                    startGeo = new com.overdrive.app.geo.GeoSnapshot(
                                            startGeoLatSnapshot, startGeoLngSnapshot,
                                            startGeoAccuracySnapshot, startGeoAgeMsSnapshot,
                                            startGeoCapturedAtMsSnapshot, 0L);
                                } else {
                        ...
                        tempFileSnapshot.delete();
                    }
                } else if (recordingBrokenSnapshot) {
                    ...
                    File broken = new File(outputPathSnapshot + ".broken");
                    if (!finalizeToDestination(tempFileSnapshot, broken)) {
                        logger.warn("Quarantine rename failed; deleting broken tmp: " + tempFileSnapshot.getName());
                        tempFileSnapshot.delete();
                    } else {
                        logger.warn("Quarantined broken recording (stopOk=" + stopOkSnapshot
                                + ", writerAborted=" + abortedAtCloseSnapshot
                                + ", " + (broken.length() / 1024) + " KB): " + broken.getName());
                    }
                } else {
                    // Empty / sub-1KB recording — drop it silently.
                    logger.warn("Deleting empty/corrupt temp file: " + tempFileSnapshot.getName() +
                            " (frames=" + recordedFramesSnapshot + ", size=" + tempFileSnapshot.length() + ")");
                    tempFileSnapshot.delete();
                }
                ...
                // Decrement and notify any close()/release() waiter. Must be
                // in finally so an exception above still releases the join —
                // mirrors the rotation finalizer's shutdown-safety pattern.
            ...
            }, "GpuEventFinalizer-" + segmentNumberSnapshot);
            finalizerThread.setDaemon(true);
            finalizerThread.setPriority(Thread.NORM_PRIORITY - 1);
    ...
    /**
     * Compact encoder/muxer/GC snapshot for stall logs on the GL thread and
     * the disk-writer thread. Safe to call from either; queue size is the
     * concurrent deque's size().
     */
    public String stallSnapshot() {
        long now = System.currentTimeMillis();
        long diskAge = (lastDiskWrittenMs > 0L) ? (now - lastDiskWrittenMs) : -1L;
        return "q=" + muxerWriteQueue.size() + "/" + MUXER_WRITE_QUEUE_CAPACITY
                + " writing=" + isWritingToFile
                + " diskAgeMs=" + diskAge
                + " vDrops=" + muxerDropCount.get()
                + " aDrops=" + audioDropCount.get()
                + blockingGcSnapshot();
    }

    private static String blockingGcSnapshot() {
        try {
            String count = android.os.Debug.getRuntimeStat("art.gc.blocking-gc-count");
            String timeMs = android.os.Debug.getRuntimeStat("art.gc.blocking-gc-time");
            if (count == null && timeMs == null) return "";
            return " blockingGcCount=" + count + " blockingGcMs=" + timeMs;
        } catch (Throwable ignored) {
            return "";
        }
    }
    ...
    /**
     * Log a real (sub-10 s) inter-frame PTS gap that will appear in the mp4
     * stts box. Clock-domain jumps are handled separately above. Gaps ≥500 ms
     * always log; 150–500 ms is rate-limited to 1 Hz so a chronic stutter
     * does not drown the daemon log.
     */
    private void maybeLogInterframeStall(long sourceGapUs) {
        long now = System.currentTimeMillis();
        boolean always = sourceGapUs >= STALL_LOG_ALWAYS_US;
        if (!always && (now - lastInterframeStallLogMs) < STALL_LOG_MIN_INTERVAL_MS) {
            suppressedInterframeStallLogs++;
            return;
        }
        int suppressed = suppressedInterframeStallLogs;
        suppressedInterframeStallLogs = 0;
        lastInterframeStallLogMs = now;
        logger.warn("Interframe stall: gap=" + (sourceGapUs / 1000L) + "ms "
                + stallSnapshot()
    ...
    private void handleWriterRotatePacketInner(MuxerPacket ticket) {
        ...
        File newTempFile = stagingFileFor(newPath);
          ...
          try {
            ...
            if (oldTemp != null && oldTemp.exists()) {
                if (!segmentBroken && oldRecordedFrames > 0 && oldTemp.length() > 1024) {
                    ...
                    if (finalizeToDestination(oldTemp, finalFile)) {
                    ...
                    if (!finalizeToDestination(oldTemp, broken)) {
    ...
    private void quarantineAbortedMuxer() {
        synchronized (muxerLock) {
            ...
            if (tempFile != null && tempFile.exists()) {
                ...
                if (finalizeToDestination(tempFile, broken)) {
