...
public class PanoramicCameraGpu {
    ...
    private static final long STALL_LOG_ACQUIRE_NS = 150_000_000L;
    private static final long STALL_LOG_ACQUIRE_ALWAYS_NS = 500_000_000L;
    private static final long STALL_LOG_MIN_INTERVAL_MS = 1_000L;
    private long lastAcquireStallLogMs = 0L;
    private int suppressedAcquireStallLogs = 0;
    ...
    private void maybeLogAcquireStall(long acquireNs) {
        long now = System.currentTimeMillis();
        boolean always = acquireNs >= STALL_LOG_ACQUIRE_ALWAYS_NS;
        if (!always && (now - lastAcquireStallLogMs) < STALL_LOG_MIN_INTERVAL_MS) {
            suppressedAcquireStallLogs++;
            return;
        }
        int suppressed = suppressedAcquireStallLogs;
        suppressedAcquireStallLogs = 0;
        lastAcquireStallLogMs = now;
        SurveillanceEngineGpu localSentry = sentry;
        String sentryDiag = (localSentry != null)
                ? localSentry.stallDiagnostics()
                : "sentry=null";
        HardwareEventRecorderGpu enc = encoder;
        String snap = (enc != null) ? enc.stallSnapshot() : "encoder=null";
        logger.warn("Acquire stall: acq=" + (acquireNs / 1_000_000L)
                + "ms " + sentryDiag + " " + snap
    ...
    private void renderLoop() {
        ...
        try {
            ...
            long acquireNs = stageAfterAcquireNs - stageT0;
            if (acquireNs >= STALL_LOG_ACQUIRE_NS) {
                maybeLogAcquireStall(acquireNs);
            }
            frameCounter++;
