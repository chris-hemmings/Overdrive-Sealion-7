...
public class OemDashcamPipeline {
    ...
    private static final long STALL_LOG_SWAP_NS = 150_000_000L;
    private static final long STALL_LOG_SWAP_ALWAYS_NS = 500_000_000L;
    private static final long STALL_LOG_MIN_INTERVAL_MS = 1_000L;
    private long lastSwapStallLogMs = 0L;
    private int suppressedSwapStallLogs = 0;
    ...
    private void maybeLogSwapStall(long swapNs) {
        long now = System.currentTimeMillis();
        boolean always = swapNs >= STALL_LOG_SWAP_ALWAYS_NS;
        if (!always && (now - lastSwapStallLogMs) < STALL_LOG_MIN_INTERVAL_MS) {
            suppressedSwapStallLogs++;
            return;
        }
        int suppressed = suppressedSwapStallLogs;
        suppressedSwapStallLogs = 0;
        lastSwapStallLogMs = now;
        HardwareEventRecorderGpu enc = encoder;
        String snap = (enc != null) ? enc.stallSnapshot() : "encoder=null";
        logger.warn("OEM swap stall: swap=" + (swapNs / 1_000_000L) + "ms " + snap
        ...
        while (running.get()) {
            try {
                ...
                if (stride <= 1 || (renderStridePhase++ % stride) == 0) {
                    ...
                    long tSwap0 = System.nanoTime();
                    eglCore.swapBuffersWithTimestamp(encoderEglSurface, ptsNs);
                    long swapNs = System.nanoTime() - tSwap0;
                    if (swapNs >= STALL_LOG_SWAP_NS) {
                        maybeLogSwapStall(swapNs);
                    }
