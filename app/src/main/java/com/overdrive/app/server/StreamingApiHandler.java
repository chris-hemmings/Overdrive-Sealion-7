...
public class StreamingApiHandler {
    ...
    // Live-view encoder+scaler init while 4 cams are recording can take
    // well over the in-app WebView's sync bridge timeout (MediaCodec
    // create/config on a busy Venus block, then a GL-queue wait). A phone
    // browser over Tailscale waits longer and succeeds; the HU toast was
    // enable_failed. Same cold-start pattern as panoStartInFlight: spawn
    // once, reply starting=true, let the client poll.
    private static final java.util.concurrent.atomic.AtomicBoolean streamEnableInFlight =
        new java.util.concurrent.atomic.AtomicBoolean(false);
    ...
    public static boolean handle(String method, String path, String body, OutputStream out) throws Exception {
        // Method-agnostic: the in-app WebView's shouldInterceptRequest used
        // to default the outbound verb to GET. Live View auto-starts a
        // POST /api/stream/enable before the fetch() patch lands, so that
        // request arrived here as GET, missed the POST-only gate, 404'd,
        // and the page toasted "Failed to enable streaming" — while a
        // Tailscale browser (real POST, no intercept) worked. Enable is
        // idempotent; accept whatever verb arrives. Same rationale as
        // /api/bs/enable.
        ...
        if (path.startsWith("/api/stream/quality/")) {
    ...
    private static void handleEnableStreaming(OutputStream out) throws Exception {
        ...
        if (streamEnableInFlight.compareAndSet(false, true)) {
            CameraDaemon.log("handleEnableStreaming: starting async enable quality=" + q.displayName);
            final GpuSurveillancePipeline p = pipeline;
            final GpuPipelineConfig.StreamingQuality quality = q;
            new Thread(() -> {
                try {
                    // enableStreaming fires the pipeline's streamStateListener →
                    // RMM.reconcileCameraProfile, which floors the global camera
                    // fps at the stream fps if the camera was parked at the
                    // BS-only idle rate.
                    p.enableStreaming(quality.width, quality.height, quality.fps, quality.bitrate);
                    CameraDaemon.log("handleEnableStreaming: success");
                } catch (Throwable t) {
                    CameraDaemon.log("handleEnableStreaming: error - " + t.getMessage());
                } finally {
                    streamEnableInFlight.set(false);
                }
            }, "StreamEnable").start();
        } else {
            CameraDaemon.log("handleEnableStreaming: enable already in flight");
        }
