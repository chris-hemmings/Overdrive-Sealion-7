...
public class GearMonitor {
    ...
    public synchronized void start() {
        ...
        try {
            ...
            int initialGearRead = readFromCarSvc();
            if (!isValidGearMode(initialGearRead) && getGearMethod != null && gearboxDevice != null) {
                try {
            ...
            pollThread = new Thread(() -> {
                while (isRunning) {
                    try {
                        ...
                        // DiLink 5: car_service BEFORE CarAdapter / gearbox HAL.
                        // Those getters often return a stuck-valid P, which would
                        // skip every fallback and freeze trip detection in Park.
                        if (!isValidGearMode(gear)) {
                            int carSvcGear = readFromCarSvc();
                            if (isValidGearMode(carSvcGear)) {
                                gear = carSvcGear;
    ...
    private int readFromCarSvc() {
        try {
            if (!com.overdrive.app.byd.DiLink5Platform.isActive()) return -1;
            int gear = com.overdrive.app.byd.CarSvcTelemetry.INSTANCE.gearValue();
            return isValidGearMode(gear) ? gear : -1;
        } catch (Throwable ignored) {
            return -1;
        }
    }
    ...
    private int readGearFromDumpsys() {
        ...
        try {
            ...
            // 2. Fallback: shared car_service dump on DiLink 5; other platforms
            // keep a dedicated grep because CarSvcTelemetry is gated off.
            if (com.overdrive.app.byd.DiLink5Platform.isActive()) {
                String dump = com.overdrive.app.byd.CarSvcTelemetry.INSTANCE.dumpsysText();
                int parsed = com.overdrive.app.byd.CarSvcTelemetry.INSTANCE.parseGearFromText(dump);
                if (isValidGearMode(parsed)) {
                    lastDumpsysReadTime = now;
                    lastDumpsysGear = parsed;
                    return parsed;
                }
                // GEAR_R / SHIFT_MODE often have no lastEvent while parked.
                // Do not scan unrelated int32Values in the dump — that is
                // what made overlay gear flicker R/N/P. Speed 0 → Park.
                int spd = com.overdrive.app.byd.CarSvcTelemetry.INSTANCE.resolvedSpeedKmh();
                if (spd == 0) {
                    lastDumpsysReadTime = now;
                    lastDumpsysGear = GEAR_P;
                    return GEAR_P;
                }
                return -1;
            }
