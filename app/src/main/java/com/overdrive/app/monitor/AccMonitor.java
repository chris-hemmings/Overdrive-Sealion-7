...
public class AccMonitor {
    ...
    /**
     * Classify a dumpsys {@code Power Mute State} {@code current} line.
     *
     ...
     */
    public static Boolean classifyDiLink5PowerModeCurrentLine(String line) {
        return DiLink5PowerMode.classifyCurrentLine(line);
    }
    ...
    public static boolean probeAccState(android.content.Context context) {
        ...
        // We probe dumpsys car_service PowerMode, then gear/speed as a driving fail-safe.
        if (com.overdrive.app.byd.DiLink5Platform.isActive()) {
            try {
                String carServicePower = com.overdrive.app.byd.CarSvcTelemetry.INSTANCE.dumpsysText();
                Boolean powerModeInUse =
                        DiLink5PowerMode.classifyCurrentLine(carServicePower);
                if (Boolean.FALSE.equals(powerModeInUse)) {
                    accOn = false;
                    inSentryMode = true;
                    ...
                    return true;
                } else if (Boolean.TRUE.equals(powerModeInUse)) {
                    accOn = true;
                    inSentryMode = false;
                    lastProbeTrustworthy = true;
                    accOnAuthoritative = true;
                ...
                // Gear / speed fail-safe. Do NOT treat a dark display or locked
                // doors as ACC-off: Overdrive's own park-darken loop turns the
                // panel off, and BYD auto-locks while driving.
