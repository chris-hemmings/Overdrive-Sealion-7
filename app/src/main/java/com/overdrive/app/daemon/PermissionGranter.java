 ...
 * plus AppOps-backed modes that `pm grant` does not flip (SYSTEM_ALERT_WINDOW).
 ...
 * 
 * pm grant / appops set work from UID 2000 (shell) which is what our daemons
 * run as. Install-time / signature permissions will be silently skipped.
 */
public final class PermissionGranter {
    ...
    public static void grantAllPermissions(String packageName) {
        ...
        grantThread = new Thread(() -> {
            ...
            // AppOps, not pm grant. dumpsys reports SYSTEM_ALERT_WINDOW as
            // already held (install permission granted=true) even when AppOps
            // is still default/ignored, so this must not wait on the skip loop.
            grantOverlayAppOp(packageName);
    ...
    /**
     * SYSTEM_ALERT_WINDOW is AppOps-backed. {@code pm grant} does not reliably
     * flip {@code OPSTR_SYSTEM_ALERT_WINDOW} to ALLOWED, and {@code dumpsys
     * package} reports it {@code granted=true} as an install permission even
     * when AppOps is still default/ignored — so the skip-already-held path
     * would never even attempt {@code pm grant}. OverlayPermissionChecker
     * (and Settings.canDrawOverlays) both consult AppOps, so this must run
     * unconditionally from the shell-UID daemon, not from the app process.
     */
    private static void grantOverlayAppOp(String packageName) {
        try {
            Process process = Runtime.getRuntime().exec(new String[]{
                    "sh", "-c",
                    "appops set " + packageName + " SYSTEM_ALERT_WINDOW allow 2>&1"});
            ...
            String out = output.toString().trim();
            if (exitCode == 0) {
                log("appops SYSTEM_ALERT_WINDOW allow");
            } else {
                log("appops SYSTEM_ALERT_WINDOW failed (exit=" + exitCode
                        + (out.isEmpty() ? ")" : "): " + out));
            }
        } catch (InterruptedException e) {
            Thread.currentThread().interrupt();
            log("appops SYSTEM_ALERT_WINDOW interrupted");
        } catch (Exception e) {
            log("appops SYSTEM_ALERT_WINDOW failed: " + e.getMessage());
        }
    }
    ...
    public static String getAdbCommands(String packageName) {
        ...
        sb.append("\n# AppOps-backed overlay permission (pm grant is not enough):\n");
        sb.append("adb shell appops set ").append(packageName)
