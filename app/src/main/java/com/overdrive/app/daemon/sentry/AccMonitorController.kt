...
class AccMonitorController(
        ...
        pollingThread = Thread({
            ...
            while (running) {
                try {
                    ...
                    Thread.sleep(if (dilink5) 1000 else 500)
                    pollCount++

                    // Every ~30s, log all sources for debugging
                    if (pollCount % (if (dilink5) 30 else 60) == 0) {
                        logAllPowerSources()
                    }
                    ...
                    // Check Display Power & Interactive State (non-DiLink 5)
    ...
    /**
     * DiLink 5 ACC for deferred-nav: Power Mute from the shared
     * [com.overdrive.app.byd.CarSvcTelemetry] dump. Do not dumpsys power or
     * treat a dark panel as ACC-off — park-darken turns the HU off while
     * driving/parked-with-ACC-on.
     */
    private fun pollDiLink5Acc() {
        val dump = com.overdrive.app.byd.CarSvcTelemetry.dumpsysText()
        val inUse = com.overdrive.app.monitor.DiLink5PowerMode.classifyCurrentLine(dump)
        val isAccOffNow = when (inUse) {
            java.lang.Boolean.FALSE -> true
            java.lang.Boolean.TRUE -> false
            else -> {
                val accAnim = execShell("getprop sys.accanim.status").trim()
                accAnim == "1"
            }
        }
        val wasAccOff = lastAccAnimStatus != "0"
        if (isAccOffNow != wasAccOff) {
            logger.info(">>> ACC STATE CHANGED (DiLink5 Power Mute): isAccOffNow=$isAccOffNow")
            if (isAccOffNow) onAccOff() else onAccOn()
            lastAccAnimStatus = if (isAccOffNow) "1" else "0"
        }
    }
