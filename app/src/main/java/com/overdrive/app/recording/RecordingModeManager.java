...
public class RecordingModeManager {
    ...
    public void resyncFromHardware(String reason) {
        ...
        synchronized (this) {
            ...
            //
            // PROXIMITY_GUARD is intentionally NOT excluded here, unlike
            // encoderStalled above. That exclusion exists because
            // PROXIMITY_GUARD legitimately goes long idle stretches with no
            // encoded frames between triggers — but this probe is already
            // gated on pipeline.isRecording()==true, which is false during
            // those idle stretches. So the idle-false-positive rationale
            // that justifies excluding PROXIMITY_GUARD elsewhere does not
            // apply to this check: it can only ever fire while a recording
            // session is genuinely open. Confirmed live (2026-09-04): a
            // PROXIMITY_GUARD event wedged mid-recording — frameCount kept
            // climbing while the .mp4 on disk stayed byte-for-byte frozen —
            // and went completely undetected because this signal, alone
            // among the wedge probes, used to blanket-exclude this mode too.
            ...
            // PROXIMITY_GUARD's blanket exclusion stays for the other three
            // signals (!recordingHealthy / pendingPrefixStuck / encoderStalled
            // — none of those are gated on pipeline.isRecording(), so they'd
            // false-positive during that mode's normal long idle waits
            // between triggers). diskStalled is the one exception: it's
            // gated on pipeline.isRecording()==true, so it is structurally
            // incapable of firing during an idle wait — only during a
            // genuinely open, genuinely stuck recording session, which is
            // exactly the wedge PROXIMITY_GUARD needs to be able to detect
            // and self-heal from (see diskStalled's definition above).
            ...
            // slot. PROXIMITY_GUARD never records continuously, so it
            // historically counted as healthy purely from modeActive=true —
            // correct back when wedge tracking was fully suppressed for
            // this mode (wedgeDetected could never be true for it). Now
            // that diskStalled's mid-recording wedge signal reaches
            // wedgeDetected for PROXIMITY_GUARD too, this must also require
            // !wedgeDetected — otherwise a genuinely wedged PROXIMITY_GUARD
            // tick would be BOTH wedgeDetected=true (scheduling a retry) AND
            // tickHealthy=true (immediately resetting the failure counter
            // that same retry just incremented), defeating the backoff/cap
            // for exactly the mode that most needed it added.
