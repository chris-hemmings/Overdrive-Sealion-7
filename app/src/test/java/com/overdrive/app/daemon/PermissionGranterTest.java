...
public class PermissionGranterTest {
    ...
    @Test
    public void adbCommandsIncludeOverlayAppOpsBecauseDumpsysGrantedIsNotAppOps() {
        // The DUMPSYS fixture above reports SYSTEM_ALERT_WINDOW granted=true.
        // That is an install-permission marker, not AppOps ALLOWED — overlays
        // still will not draw until `appops set ... SYSTEM_ALERT_WINDOW allow`.
        String cmds = PermissionGranter.getAdbCommands("com.overdrive.app");
        assertTrue(cmds.contains(
                "appops set com.overdrive.app SYSTEM_ALERT_WINDOW allow"));
    }
}
